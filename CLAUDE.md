...
> 最後更新：2026-06-08（**Session 7**）。**接手第一件事：讀 `HANDOFF.md` 開頭「★ Session 7 交接」段**。Session 7 摘要：離線預算 `career_budget`（50 職涯預算進 Postgres，命中秒回 0 即時 AI）＋柴犬等候動畫（Lottie 三色、done-driven 數字）＋Q&A 失敗復原（過載重試泡泡/查無資料引導）＋Sentry 降噪（暫時性 Gemini 錯誤降 warning、本機不啟用）＋多輪追問修復（history-based 統一）。**⚠️ 留下一個高優先未修 bug：POST /recommend 的 `asyncio.to_thread`(commit 21e6a93) 在 worker thread 跑 `asyncio.run` → 用完即關的 loop 污染共用 `_client.aio` → 之後所有 `/recommend/stream`(清單外)、`/qa/stream` 噴「Event loop is closed」。正解＝POST /recommend 改 async 直接 await pipeline（見 HANDOFF Session 7 🔴 與設計決策 #22）。** 本機 master **15 commits ahead 未 push**；要部署＝git push。後端 254 測試綠、前端 `?v=34`。注意 `~/.claude` 的 memory 不會跨機器，必要資訊都在 HANDOFF/CLAUDE。
>
...
22. **（Session 7）⚠️ 未修 bug：`asyncio.to_thread` + 共用 async client 跨 loop 污染**：POST /recommend 即時路徑用 `build_recommendation_instrumented`（同步、內部 `asyncio.run(_run())`，`_run` 用模組層共用 `_client.aio`）。為修「asyncio.run in running loop」崩潰，commit `21e6a93` 改 `await asyncio.to_thread(build_recommendation_instrumented,...)` → 但 worker thread 的 `asyncio.run` 開**用完即關的 loop**，把共用 `_client.aio` 的 httpx client **綁到死 loop** → 之後主 loop 上所有 `/recommend/stream`(清單外)、`/qa/stream` 噴 **「Event loop is closed」**。**＝把單請求崩潰換成污染整個 worker 的更廣 regression**（單元測試/code-review 沒抓到，因沒測「同程序下一個 async 請求」；e2e 才抓到）。**正解：POST /recommend 改 async、直接 `await` 一個 `build_recommendation_instrumented_async`（全程主 loop，不開新 loop、不 to_thread）**。`stream_recommendation` 本就 async 在主 loop，故新鮮程序第一個打 stream 通常會動；bug 只在「POST 即時路徑跑過後」浮現。修完要過真後端 e2e：先打一次 POST 清單外 → 再打 stream/qa 確認不再 closed。詳見 HANDOFF Session 7 🔴。
